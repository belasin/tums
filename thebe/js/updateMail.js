updateMail = function () {
    try {
        var myIn = getElement('user-userSettings-uid').value;
    }
    catch (dc) {
...
showDomains = function() {
    try {
        showElement(getElement('user-tumsUser-field').childNodes[1]);
    } catch(dc){}
...
function rollForwards(){
    ...
    for (i=1; i<10; i++){
        ...
        try {
            name = 'user-mailSettings-mailForwardingAddress'+i+'-field';
    ...
    for (i=1; i<10; i++){
        ...
        try {
            name = 'user-mailSettings-mailAlternateAddress'+i+'-field';
            thisone = getElement(name);
            if (thisone.childNodes[1].firstChild.value==""){
                hideElement(name);
                displayedAliases--;
...
function addForward(){
    if (displayedForwards < 10){
        ...
        try {
            name = 'user-mailSettings-mailForwardingAddress'+displayedForwards+'-field';
            showElement(name);
        } catch (d){}
    }
}
...
function addAlias(){
    if (displayedAliases < 10){
        ...
        try {
            name = 'user-mailSettings-mailAlternateAddress'+displayedAliases+'-field';
            showElement(name);
        } catch (d){}
    }
}
...
function checkAdmin(){
    try {
        if (getElement('user-tumsAdmin').checked){
            hideElement('user-tumsUser-field');
            hideElement('addDom');
        } 
        else {
            showElement('user-tumsUser-field');
            showElement('addDom');
        }
    } catch(dc) {}
...
atatchEvents = function() {
    try {
        connect('user-userSettings-uid', 'onkeyup', updateMail);
        connect
    }
    catch(dontCare) { }
    ...
    try {
        connect('user-tumsAdmin', 'onclick', checkAdmin);
    }
    catch(dontCare) {}
...
addLoadEvent(function() {
    rollForwards();
    try {
        container = getElement('user-tumsUser-field');
