...
if len(sys.argv) > 1:
    ...
    os.system("cd /root/build/Vulani/tums/; svn up")

    os.system("rm -rf /root/build/Vulani/tums/branches/rel/%s" % relnum)
    print "Creating and tagging branch"
    os.system("cp -a /root/build/Vulani/tums/trunk/source /root/build/Vulani/tums/branches/rel/%s" % relnum)
    ...
    if os.getcwd() == newwd:
        ...
        for name, filen in fi:
            ...
            os.system("/root/build/Vulani/tums/trunk/build/Python-2.5.2/Tools/freeze/freeze.py -x %s %s >> /root/build/freezeLog 2>&1" % (myList, filen))
            #cmd = "/usr/bin/cxfreeze --exclude-modules=%s %s" % (','.join(modList), filen)
            #os.system(cmd)
            print ".", 
