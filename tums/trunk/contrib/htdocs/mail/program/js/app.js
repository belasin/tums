/*
 ...
 +-----------------------------------------------------------------------+
  ...
  $Id: app.js 297 2006-08-06 15:55:11Z thomasb $
*/
...
function rcube_webmail()
    ...
    switch (this.task)
      ...
      case 'mail':
        ...
        if (this.env.action=='show')
          {
          this.enable_command('show', 'reply', 'reply-all', 'forward', 'moveto', 'delete', 'viewsource', 'print', 'load-attachment', 'newwin', true);
  ...
  this.highlight_row = function(id, multiple)
	...
	this.enable_command('delete', 'moveto', this.selection.length>0 ? true : false);
        ...
        {
        this.enable_command('show', 'reply', 'reply-all', 'forward', 'print', 'newwin', selected);
        this.enable_command('delete', 'moveto', this.selection.length>0 ? true : false);
        }
      }
    else if (this.task=='addressbook')
...
*/
  ...
  // Opens mail in a new window...
  this.open_new_window = function(){
    var uid;
    if (uid = this.get_single_uid()) {
      thisLoc = this.env.comm_path+'&_action=show&_uid='+uid+'&_mbox='+escape(this.env.mailbox);
