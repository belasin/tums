        ...
        for iface, vals in Utils.getIFStat().items():
            ...
            if self.ifdisp.get(iface):
                ...
                if (nin+nout) > 0:
                    ...
                    l.write('%s:%s' % (self.iftraf[iface][0], self.iftraf[iface][1]))
                    l.close()
    ...
    def check_mailq(self):
        ...
        def parse(queue):
            try:
                ...
                return
            if int(vol) > 400:
                self.sendAlert("Large mail backlog detected. %s messages await delivery" % vol, 'vol1', sub="Vulani notice mq=%s " % vol)
                return 
            if size[-2:] == "KB":
                bsize = bsize * 1024
            if size[-2:] == "MB":
            ...
            if bsize > 104857600: # 100 MB.
                self.sendAlert("Large mail backlog detected. Queue size has reached %s" % size, 'vol2', sub="Vulani notice mqs=%s " % vol)
                return
