...
class EditDNS(Tools.Page):
    ...
    def submitZone(self, ctx, form, data):
        ...
        G['zones'][self.domain]['update'] = data['update'].encode().replace(' ','').split(',')
        G['zones'][self.domain]['ns'] = data['ns'].encode().replace(' ','').split(',')
...
class DyndnsEntry(PageHelpers.DataTable):
    ...
    def addAction(self, data):
        ...
        entries.append([
            self.provMap[data['provider'].encode()], 
            data['server'].encode(), 
        ...
        return ctx.tag[
            ...
            tags.div(id="panelDNSZone", _class="tabPane")[
                ...
                PageHelpers.dataTable(["Zone", "Master", "NS", ''],
                    [
                        [
                            i, "type master" in self.sysconf.General['zones'][i].get('options', []), 
