...
class AddNeigh(Tools.Page):
    ...
    def submitNeigh(self, ctx, form, data):
        ...
        if data['asn']:
            dta['as'] = data['asn'].encode()

        if data['multihop']:
            dta['multihop'] = True

        if data['nexthop']:
            dta['nexthop'] = data['nexthop'].encode()

        B[self.asn]['neighbors'][data['ip'].encode()] = dta
...
class Page(Tools.Page):
    ...
    def submitAS(self, ctx, form, data):
        ...
        if B.get(data['as'].encode(), False):
            B[data['as'].encode()]['router-id'] = data['id'].encode()
        else:
            B[data['as'].encode()] = {
                'networks': [],
                'neighbors': {},
                'router-id': data['id'].encode()
            }
        self.sysconf.BGP = B
    ...
    def submitProxyARP(self, ctx, form, data):
        ...
        if B.get('proxyarp', False):
            B['proxyarp'].append([data['ip'].encode(), data['intif'].encode(), data['extif'].encode()])
        else:
            B['proxyarp'] = [[data['ip'].encode(), data['intif'].encode(), data['extif'].encode()]]
    ...
    def submitSource(self, ctx, form, data):
        ...
        if data['dest']:
            dest = data['dest'].encode()
        else:
            dest = '0.0.0.0/0'

        if data['port']:
            port = data['port'].encode()
        else:
            port = '-'
