...
# Python
import sys, sha, md5, time, binascii, struct, exceptions
...
class ThebeMessageHandler(proto.CDPMessageHandler):
    ...
    def createKey(self, key):
        ...
        # Fold the MD5 in half with an xor of each part
    ...
    def decodeMessage(self, key, message):
        ...
        #Make sure we always have a positive value without overflow on the sign
        mycrc = binascii.crc32(m[16:16+dsize]) & 0xffffffff
        if crc  == mycrc:
            # Old method supported here for backward compatibility
            return m[16:16+dsize]
        elif crc == int(dsize):
            # New parity block check.
            return m[16:16+dsize]
        else:
            return None
