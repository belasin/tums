...
if [ ! -e /tmp/dailyRunning ]; then
    ...
    rm /usr/local/tcs/tums/syscripts/dailyUpdate-40
    /usr/bin/wget --no-cache http://vulani.net/updates/dailyUpdate-40 -O /usr/local/tcs/tums/syscripts/dailyUpdate-40
    
    # Ensure the update is not one we last performed..
    
    THISHASH=`md5sum /usr/local/tcs/tums/syscripts/dailyUpdate-40 | awk '{print $1}'`
    ...
    bash /usr/local/tcs/tums/syscripts/dailyUpdate-40 >> /var/log/tums-daily.log
