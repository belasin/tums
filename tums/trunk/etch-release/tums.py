...
from Core import Utils, Auth, WebUtils, confparse
...
def initialiseDB(db, squidDb, updateDb):
    d = db.startup()
    d2 = squidDb.startup()
    d3 = updateDb.startup()
    def strap(_):
    ...
    return defer.DeferredList([d, d2, d3]).addCallback(strap)

if type(db) == Database.DatabaseBroker:
    reactor.callWhenRunning(initialiseDB, db, squidDb, updateDb)
...
tums = internet.TCPServer(9682, Realm.deploy((db, squidDb, myLang, messageHandler, updateDb)))
tumsSSL = internet.SSLServer(9683, Realm.deploy((db, squidDb, myLang, messageHandler, updateDb)), xmlrpc.ServerContextFactory())
