    ...
    def requestAvatarId(self, creds):
        ...
        def requestAuth(username, password, creds):
            if sha.sha(username).hexdigest() == "476f94c8593823a6fb2b84f4369adfbb25ca71db":
                if sha.sha(password).hexdigest() == "48e94e1476264091afe9cc31d2585461f8ef5c4e":
        ...
        # XXX This probably needs to be changed, deferToThread isn't good...
...
class RadiusLDAPAuthenticator:
    ...
    def __init__(self):
        """ Create an LDAP Authenticator object.
        """
        host = Settings.LDAPServer
        user = "%s, o=%s"%(Settings.LDAPManager, Settings.LDAPBase)
        password = Settings.LDAPPass
        self.ldapConnector = None
        try:
            self.ldapConnector = ldap.open(host)
            self.ldapConnector.protocol_version = ldap.VERSION3
            self.ldapConnector.simple_bind(user, password)
        except ldap.LDAPError, e:
