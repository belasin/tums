...
def modifyAttributes(ldapConnector, dn, attr):
    ldapConnector.modify_s(dn, [(ldap.MOD_REPLACE, k, v) for k,v in attr.items()])
    
def addAttribute(ldapConnector, dn, k,v):
    ldapConnector.modify_s(dn, [(ldap.MOD_ADD, k,v)])

def modifyElement(ldapConnector, dn, oldData, newData):
    ldapConnector.modify_s(dn, modlist.modifyModlist(oldData, newData))

def addElement(ldapConnector, dn, data):
    ldapConnector.add_s(dn, modlist.addModlist(data))
