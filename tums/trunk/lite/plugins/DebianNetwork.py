            ...
            if interface != "extra" and "vlan" not in interface:
                ...
                if defin['type'].lower() == 'dhcp':
                    ...
                    if interface == config.LANPrimary and config.Tunnel.get('ipv6', False):
                        ...
                        net+= '  up ip tunnel add ipv6tun mode sit remote %s local %s ttl 255 || true\n' % (remote, local)
                        net+= '  up ip link set ipv6tun up || true\n'
                        net+= '  up ip addr add %s dev ipv6tun || true\n' % (localv6) 
                        net+= '  up ip -6 ro add 2003::/3 dev ipv6tun || true\n'
