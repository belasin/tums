...
def restartSnom(macAddr):
    return WebUtils.system('asterisk -r -x "sip notify reboot-snom %s"' % macAddr)
...
class Queues(PageHelpers.DataTable):
    ...
    def addForm(self, form):
        ...
        form.addField('announce', formal.Boolean(), label = "Announce", description = "Announnce queue posistion and wait time every 30 seconds")
...
class VoipProvider(PageHelpers.DataTable):
    ...
    def addAction(self, idata):
        ...
        cb = {
            ...
            'context': idata['context'].encode('ascii', 'replace'), 
            'branch': idata['branch'], 
            'trunk': idata['trunk'], 
...
class Page(Tools.Page):
    ...
    def form_pbxConfig(self, data):
        ...
        if False:
            form.addField('addPCodec', formal.Boolean(), label = "Enable g729/g723",
                description = [
                    "By selecting the enable button you enable g729 and g723 codecs for this server", tags.br,
                    tags.b["Warning:"],
                    "To use G.729 or G.723.1 you may need to pay a royalty fee. Please see http://www.sipro.com for details. If a patent exists in your country for G.729 or G.723.1 then you should contact the owner of that patent and request their permission before executing the code."])
    ...
    def render_extensions_old(self, ctx, data):
        extensions = self.sysconf.PBXExtensions
        ...
        for k in extensions:
            ...
            tabList.append((k.capitalize(), "panelExten%s" % k))

            tabs.append(
                tags.div(id="panelExten%s" % k, _class="tabPane")[
                    ""
                ]
            )
        ...
        return ctx.tag[
            ...
            tags.div(id="panelMainGroups", _class="tabPane")[
                PageHelpers.dataTable([('istr', 'Extension Group'), ('istr', '')], dt, sortable=True), 
                tags.h3['Add Extension Group'], 
                tags.directive('form addExtensionGroup')
            ],
            tabs,
    ...
    def render_extensions(self, ctx, data):
        tabList = [
            ...
            #("Switchboard Menu", "panelIVR"),
            ("Extension Sets", "panelExten"),
            ("Number Routing", "panelRouting"),
        ]
        ...
        for k in numberRouters:
            ...
            for ky, num in enumerate(res):
                row = (
                    ...
                    tags.a(href='DelExtenGroup/%s/%s' % (k,ky))[tags.img(src="/images/firewall/delete-icon-hover.png")]
                )
                pd.append(row)
            ...
            tabs.append(
                    tags.div(id="panelRouter%s"%k, _class="tabPane")[
                        ...
                        tags.h3['Add Number Expression to ' + k],
    ...
    def addHandset(self, ctx, form, data):
        ...
        cblock = {
            'callerid': (data['callerid'] or u"%s" % user).encode('ascii', 'replace'),
            'username': user, 
            'phone':{
                'type': data['phonetype']
            },
            'secret': data['password'].encode('ascii', 'replace'),
            'call-limit': data['calllimit'] or 0
        }

        if data['phonetype'] == "Snom 320":
            fkeys = []
            for i in range(12):
                kn = 'Snom320fkeys%s'%i
                if kn not in data:
                    continue
                if data[kn]:
                    fkeys.append('%s' % (data[kn].encode('ascii', 'replace')))
            
            cblock['phone']['fkeys'] = fkeys
            cblock['phone']['mac'] = ""
            if data['Snom320MAC']:
                cblock['phone']['mac'] = data['Snom320MAC'].encode('ascii','replace').replace('-', ':').lower()
            
            cblock['phone']['type'] = "Snom 320"
    ...
    def customForm_editIVR(self, data, strData):
        ...
        form = formal.Form()
        extenWidget = formal.widgetFactory(formal.SelectChoice, options=PBXUtils.getAvaExtenNumSelect(True,[])) #provide a list of current extensions
    ...
    def childFactory(self, ctx, seg):
        ...
        return super(Page, self).childFactory(self,ctx,seg)
