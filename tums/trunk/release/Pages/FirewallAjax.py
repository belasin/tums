...
class FirewallFragment(formal.ResourceMixin, athena.LiveFragment):
    ...
    def renderRule(self, rule):
        ...
        return [
            tags.td[""],                 # move
            tags.td[rule[0]],            # ruletype
            tags.td['Source', tags.br, 'Destination'],
            tags.td[
                rule[1],                # Source Zone
                tags.br,
                rule[4]                 # Destination Zone
            ],
            tags.td[
                rule[3].replace('-', 'Any'),    # Source port
                tags.br,
                rule[7].replace('-','Any')      # Destination port
            ],
            tags.td[
                rule[2],                # Source IP
                tags.br,
                rule[5]                 # Destination IP
            ],
            tags.td[rule[6].replace('-', 'Any')],   # Protocol
            tags.td[""],                            # Edit col
            tags.td[""],                            # delete col
        ],
    ...
    def render_firewallRules(self, ctx, data):
        ...
        outRules = [tags.tr(id="fwrule_%s" % n, _class="dynRow")[self.renderRule(rule)] for n, rule in enumerate(rules['AIP'])]
        return tags.div(id="firewallRulesTab", _class="tabPane")[
            ...
            tags.table(id="firewallRulesHeader")[
                ...
                tags.tbody(id="firewallRules", _class="movableTbody")[
                    outRules
                ]
            ],
 
            #tags.ul(id="firewallRules", _class="movableTable")[outRules],
    ...
    def renderDataTable(self, id, rulePrefix, data, columns, colClasses):
        ...
        """
        rulesTags = [tags.li(id="%s_%s" % (rulePrefix, n),
                    _class= (n % 2 == 0) and "odd" or "even")[
        ...
        return [ 
            tags.table(id=id+"_header", _class="tableHeaderRow")[
                tags.tr[tableData]                
            ],
    ...
    def submitForwardPort(self, data):
        ...
        self.callRemote('insertListHTML', u'firewallForwardRules', txt_ruleID, unicode(htmlOut), u'forwardRule')
    ...
    def submitRedirectRule(self, data):
        ...
        self.callRemote('insertListHTML', u'firewallRedirectRules', txt_ruleID, unicode(htmlOut), u'redirectRule')
    ...
    def submitMasqRule(self, data):
        ...
        self.callRemote('insertListHTML', u'firewallMasqRules', txt_ruleID, unicode(htmlOut), u'masqRule')
    ...
    def submitSNATRule(self, data, ruleID = None):
        ...
        self.callRemote('insertListHTML', u'firewallSNATRules', txt_ruleID, unicode(htmlOut), u'snatRule')
    ...
    def submitQosRule(self, data):
        ...
        self.callRemote('insertListHTML', u'firewallQosRules', txt_ruleID, unicode(htmlOut), u'qosRule')
    ...
    def setTransProxy(self, state=False):
        ...
        if(state):
            ...
            self.callRemote('insertListHTML', u'firewallRedirectRules', None, unicode(htmlOut), u'redirectRule')
        self.callRemote('initRedirectListEvents');
    ...
    def render_firewallZones(self, ctx, data):
        ...
        zoneColClasses = ['name', 'policy', 'log', 'interface', 'members', 'edit', 'delete'];
        ...
        return tags.div(id="firewallZonesTab", _class="tabPane")[
            ...
            zoneTableTags,
            tags.div(id="firewallZoneFormDialog", _class="dialogwin", style="width:450px;height:auto;display:none")[
    ...
    def submitZone(self, data, ruleID = None):
        print data
    athena.expose(submitZone)

    def delZone(self, ruleID = None):
        print "DELETE ", str(ruleID)
    athena.expose(delZone)

    def render_firewallConnections(self, ctx, data):
        return tags.div(id="firewallConnectionsTab", _class="tabPane")[
            tags.strong["Connections go here"]]
