...
class ClusterFragment(athena.LiveFragment):
    ...
    def lanTest(self):
        ...
        loc = 'nmap -sS -p 9682,54322 -oG - %s 2>&1 | grep "9682/open/tcp" | grep "54322/open/tcp"' % (lanIP.encode("ascii", "replace"))
        print loc
...
class Page(PageHelpers.DefaultAthena):
    ...
    def submitMaster(self, c, f, data):
        ...
        if not G.get('haconf'):
            G['haconf'] = {
                'masterkey': data['key'].encode("ascii", "replace")
            }
        else:
            G['haconf']['masterkey'] = data['key'].encode("ascii", "replace")
        self.sysconf.General = G 
