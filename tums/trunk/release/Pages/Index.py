        ...
        if segments[0] == "login":
            ...
            if self.sysconf.ProxyConfig.get('adauth', None):
                if self.sysconf.ProxyConfig.get('addom', None):
                    basedn = str.join(',',["dc="+str(dfrag) for dfrag in self.sysconf.ProxyConfig.get('addom', str).split('.')])
                    adInfo = {
                        'basedn': basedn,
                        'ldapuser': self.sysconf.ProxyConfig.get('adldapuser', ""),
                        'ldappass': self.sysconf.ProxyConfig.get('adldappass', ""),
                        'adserver': self.sysconf.ProxyConfig.get('adserver', "")
                    }
                ...
                return WebUtils.system("echo %s %s | %s" % (user, passw, cmd)).addBoth(gotADAuth)
            else:
...
class Page(rend.Page):
    ...
    child_files = compression.CompressingResourceWrapper(static.File(Settings.BaseDir+'/files/'))
