...
class ShowSearch(Reports.Page):
    #docFactory  = loaders.xmlfile('framed.xml', templateDir=Settings.BaseDir+'/templates')
    def __init__(self, avatarId = None, db = (None, None, None, None), search = [], *a, **kw):
        self.search = search
    ...
    def render_content(self, ctx, seg):
        ...
        return self.db[0].searchMessages(*self.search[:4]).addCallbacks(returnMailLog, error)

class Page(Reports.Page):
    #docFactory  = loaders.xmlfile('framed.xml', templateDir=Settings.BaseDir+'/templates')
    def __init__(self, avatarId = None, db = (None, None, None, None), offset = 0, *a, **kw):
        self.offset = offset
        Reports.Page.__init__(self,avatarId, db, *a, **kw)
    ...
    def submitForm(self, ctx, f, d):
        ...
        return url.root.child('Mail').child('Search').child(
            d['to'] or "None").child(
            d['from'] or "None").child(
            fromdate).child(
            todate)
    ...
    def render_content(self, ctx, seg):
        ...
        return self.db[0].getLastMessages(offset = self.offset).addCallbacks(returnMailLog, error)
