    ...
    def render_updates(self, ctx, data):
        def gotDetails(dlcache, totals):
            ...
            data = [
                ...
                ("Antivirus", 1 + int(totals.get('antivirus', [0,1000])[1])/1000),
            ]
            ...
            def overallSavings():
                savings = {}
                totalSaving = 1
                for i in ['microsoft', 'linux', 'antivirus']:
                    saving = 1
            ...
            categories = [
                ...
                ('linux', 'penguin.png'), 
                ('antivirus', 'proxy.png') # need a propper icon here :/
            ]

            return ctx.tag[
                PageHelpers.TabSwitcher((
                    ('Stats', 'panelStats'),
                    ('Windows', 'panelMicrosoft'),
                    ('Linux',   'panelLinux'),
                ...
                [
                    tags.div(id='panel%s' % category[0].capitalize(), _class='tabPane')[
                        tags.h3[tags.img(src="/images/%s" % category[1]), "%s update cache" % category[0].capitalize()],
                        statSet(category[0]),
                        ...
                        PageHelpers.dataTable(tabCols,
                            ...
                            ) for i in cache.get(category[0],[])[-10:]], sortable = True
                        )
        ...
        def getFiles(totals):
            ...
            for i in totals:
                todata += i[2]
                tohit += i[1]
                tots[i[0].encode("ascii", "replace")] = (i[1], i[2])
            tots['total'] = (tohit, todata)

            tots['antivirus'] = tots['av-avg']
            ...
            return defer.DeferredList([
                ...
                self.db[4].getFilesByType('av-avg')
