...
class EditXen(Tools.Page):
    ...
    def submitDomain(self, ctx, form, data):
        ...
        name = data['name'].encode("ascii", "replace")
...
class Page(Tools.Page):
    ...
    def submitNewXen(self, ctx, form, data):
        ...
        xenConfig = {
            'lva' : False, #data['lva'].encode("ascii", "replace"),
            'dist' : data['distribution'].encode("ascii", "replace"),
            'memory' : data['memory'],
            'disk' : data['disk'],
            'ip' : data['ip']
        }
        ...
        if not gen.get('xen'):
            gen['xen'] = {'images':{}}
        gen['xen']['images'][data['name'].encode("ascii", "replace")] = xenConfig
        self.sysconf.General = gen
