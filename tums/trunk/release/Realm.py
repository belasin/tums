        ...
        for iface in interfaces:
            if iface is inevow.IResource:
                if avatarId is checkers.ANONYMOUS:
                    ...
                    res = self.root(avatarId, self.db)
                    res.realm = self
...
def createResource(db, root):
    ...
    s = guard.SessionWrapper(createPortal(db, root))
    s.persistentCookies = True
    s.sessionLifetime = 86400
    return s
#--------------------------------------#
...
def deploy(db):
    ...
    @param db: C{AccessBroker} instance
    """
    guardSite = createResource(db, Menu.Page)
    calendarSite = createResource(db, Wrapper.Page)
    siteRoot = Index.Page(db)
    #siteRoot.putChild('vhost', vhost.VHostMonsterResource())
    siteRoot.putChild('auth', guardSite)
    siteRoot.putChild('mail', calendarSite)
