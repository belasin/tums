...
def mainLog():
    main = open('/var/log/exim4/mainlog')
    completeIds = []
    mstats = {}
    stats = {}
    for l in main:
        ...
        if "=>" in message or "<=" in message and "<>" not in lsegs[4]:
            ...
            if "<=" in message:
                ...
                if stats[date][messageId] ==  {}: # Mail is trash, there has to be a <= line before this is hit
                    ...
                    elif (("R=dnslookup" in message) or ("R=gateway" in message)) and stats[date][messageId]['relay'] == 'local':
...
def rejectLog():
    ...
    reject = open('/var/log/exim4/rejectlog')
...
if __name__ == '__main__':
    try:
        ...
        print "Warning: %s No reject stats will be generated" % e
        rejects = []
    ...
    for date in rejects:
        ...
        fdrs = open(Settings.BaseDir + '/statdb/%s' % fn, 'wt')
    ...
    for date in delivers:
        ...
        fdms = open(Settings.BaseDir + '/statdb/%s' % fn, 'wt')
