...
def getAvaExtenNumSelect(limit=True, includeInLimit=[]):
    ...
    for ivr, ivrDet in config.PBX.get('ivr', {}).items():
        usedExtList.extend(ivrDet['extensions'])
    for fax, fDet in config.PBX.get('faxing', {}).items():
        usedExtList.extend(fDet['extensions'])
    for ext in extList:
        ext = str(ext)
...
def getAllAvaExtDeviceEndPoints(currentDevs=[]):
    devList = getAllExtDeviceEndPoints()
    #PBXExtensions = config.PBXExtensions
    for extname, extConf in config.PBXExtensions.items():
        for devName in extConf.get('devices', []):
            if devName in devList:
                devList.remove(devName)
    devList.extend(currentDevs)
    return devList
...
def resolveKeyNumber(key):
    ...
    if 'ext/' == key[0:4]:
        if key[4:] in config.PBXExtensions:
            ...
            if ext['extensions'] and ext['devices']:
    ...
    if 'fax/' == key[0:4]:
        if key[4:] in config.PBX.get('faxing', {}):
            faxing = config.PBX.get('faxing', {})[key[4:]]
            return faxing['extensions'][0], None
    if 'conf/' == key[0:5]:
        if key[5:] in config.PBX.get('meetme', {}):
            meetme = config.PBX.get('meetme', {})[key[5:]]
            return meetme['extensions'][0], None
    if 'ivr/' == key[0:4]:
...
def getAllExtenDevs():
    ...
    for user, extension in config.PBXExtensions.items():
        devList = [ resolveDevice(dev) for dev in extension['devices'] ]
        if devList:
            output[user] = devList
    return output
...
class PBXHardware(object):
    ...
    def __init__(self):
        ...
        dirList = os.listdir("/usr/local/tcs/tums/PBXHardwarePlugins")
        for pluginFileName in dirList:
            if ".py" == pluginFileName[-3:] and not "__init__" in pluginFileName:
                try:
                    hardwarePlugin = __import__("PBXHardwarePlugins."+pluginFileName.replace(".py",""),globals(), locals(), ['plugins']).Plugin(config)
