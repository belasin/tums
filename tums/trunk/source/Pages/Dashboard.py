...
from Core import PageHelpers, AuthApacheProxy, WebUtils, Utils, NetUtils
...
class Page(PageHelpers.DefaultAthena):
    ...
    def switchProfile(self, ctx, form, data):
        ...
        l.write(data['profile'].encode("ascii", "replace"))
        l.close()
    ...
    def data_vpnclients(self, ctx, data):
        if not os.path.exists('/etc/openvpn/vpn.conf'):
            return []
        
        res = []

        try:
            n = open('/var/log/vpn-status.log')
            lastL = ""
            started = False
            started2 = False
            ...
            mtable = NetUtils.getARP()

            for i in n:
                if "Common Name" in lastL: # Start after title list
                    started = True

                if "ROUTING TABLE" in i:
                    # Do the address table
                    started = False
                    continue

                if "Virtual Address" in i:
                    started2 = True
                    continue

                if "GLOBAL STATS" in i:
                    # Kill loop here
                    break
                ...
                if started2:
                    print "S2", i
                    segs = i.strip('\n').split(',')
                    ips[segs[1]] = mtable.get(segs[0], None)
            ...
            ftable = []
            for segs in res:
                ftable.append([segs[0], segs[1], ips[segs[0]] or "Unknown", segs[2], segs[3], segs[4]])
            print ftable
            res = ftable 
        except Exception, e:
            print "Unable to grab VPN status", e
    ...
    def newLicenseKey(self, ctx, form, data):
        l = open(self.gkf(), 'wt')
        l.write(data['key'].encode("ascii", "replace") + '\n')
        l.close()
    ...
    def activatePhone(self, ctx, form, data):
        mk = self.gK()
        if mk == data['code'].encode("ascii", "replace"):
            # valid key
    ...
    def render_license(self, ctx, data):
        if os.path.exists('/etc/.vdf'):
            return tags.div(_class="roundedBlock")[
                    tags.img(src="/images/network-small.png"), 
                    tags.h1["Software License"], 
                    tags.div[
                        tags.h1["Vulani License Expired"],
                        tags.p["Your Vulani license has expired."], 
    ...
    def render_network(self, ctx, data):
        ...
        def ret(r):
            ...
            for c, i in enumerate(interfaces):
                ...
                if c > 0:   
                    scriptExtra.append("rollNet(\"%s\");" % iface)

                block =  tags.table(_class="interfaceTable", cellspacing="0", cellpadding="0", width="2em")[
                    tags.tbody[
                    tags.tr[
                        state,
                        tags.td[
                            tags.img(src='/images/block-minus.png', onclick=cl, id='netroller-%s' % iface)
                        ]
                    ],
                    tags.tr[
                        tags.td(style="border-bottom:2px solid #aaa", colspan="3")[
                            entities.nbsp, 
                            tags.table[
                                tags.tbody[
                                tags.tr[
