...
class Page(Reports.Page):
    addSlash = True
    def __init__(self, avatarId=None, db=None, graph="load", period="",*a, **kw):
    ...
    def form_selectLog(self, ctx):
        ...
        for g in graphdir:
            if "graph-" in g or 'iface-' in g:
                ...
                value = name
                if 'iface-' in g:
                    name = "Interface Traffic - %s" % name
                    value = 'iface-' + value

                elif not "load" in name:
                    continue
                else:
                    name = "System Load"
