...
class Page(rend.Page):
    ...
    butMat = { #Buttons memory support 136 buttons
        ...
        '300':[[0,1,2,3,4,5]],
        '360':[[6,0,7,1,8,2,9,3,10,4,11,5,
        12,23,13,24,14,25,15,26,16,27,17,28,18,29,19,30,20,31,21,32,22,
        33,44,34,45,35,46,36,47,37,48,38,49,39,50,40,51,41,52,42,53,43,
        ]],
    ...
    firmware = {
        'v6': 'http://provisioning.snom.com/update6to7/update_once.php?',
        'v7': 'http://provisioning.snom.com/release/firmware.php?',
        '300': {
            '6':'snom300-6.5.20-SIP-j.bin',
            '6.5.20': 'snom300-7.3.30-SIP-bf.bin',
            '7': 'snom300-7.3.30-SIP-bf.bin',
            '7.3.30': None
        },
        '360': {
            '6':'snom360-6.5.20-SIP-j.bin',
            '6.5.20': 'snom360-7.3.30-SIP-bf.bin',
            '7': 'snom360-7.3.30-SIP-bf.bin',
            '7.3.30': None
        },
        '320': {
            ...
            '7.3.30': None #Latest version does not need upgrading
        },
    }
    ...
    def genSnomFkeys(self, fkeys, type="320", useblf=True):
        ...
        output = []
        ext = 0
        for k, butNum in enumerate(self.butMat.get(type, self.butMat['320'])[ext]):
    ...
    def generateHandsetConfig(self, macAddr):
        ...
        if phoneEnt:
            ...
            if ptype == 'snom':
                ...
                if type == '300':
                    displayName = displayName[2:] #Strip out the first two '0' chars
                ...
                if self.phoneVer[0] not in self.firmware.keys():
                    ...
                    if autoVer in self.firmware:
                        fwurl = "firmware_status: %s%s" % (self.firmware[autoVer], str(time.time()))
        ...
        if self.file == 'snom-firmware.htm':
            if self.phoneVer[0]:
                ...
                if firmwareFile:
                    ...
                    output = "\nfirmware: http://%s:9682/files/snom-fw/%s\n" % (
                        self.lanIP,
                        firmwareFile,
                    )
                else:
                    print "Snom of type %s requested firmware for version %s, I have nothing newer" % (
                        self.phoneVer[0],
                        str.join('.',self.phoneVer[1]),
                    )
