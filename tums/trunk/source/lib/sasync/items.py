...
class Transactor(AccessBroker):
    ...
    the persistence of name:value items within a uniquely-identified group,
    e.g., for a persistent dictionary using L{PersistentDict}.

    My methods return Twisted C{Deferred} instances to the results of their
    ...
    def userStartup(self, null):
        """
        You can run my transaction methods when the C{Deferred} returned from
    ...
    def load(self, name):
        ...
        rows = self.s().execute(name=name).fetchone()
        if not rows:
...
class Items(object):
    ...
    def __init__(self, ID, *url, **kw):
        ...
        @keyword ID: A hashable object that is used as my unique identifier.
    ...
    def write(self, funcName, name, value, niceness=0):
        """
        Performs a database write transaction, returning a C{Deferred} to its
        completion.

        If we are updating the search index, there's a nuance to the
        C{Deferred} processing. In that case, when the write is done, the
        C{Deferred} is fired and processing separately proceeds with indexing
        of the written value. Here's how it works:
            ...
            as the callback to its C{Deferred} B{d2}. Note that the
            defer-to-queue transaction keeps a reference to the C{Deferred}
    ...
    def load(self, name):
        """
        Loads item I{name} from the database, returning a C{Deferred} to the
    ...
    def loadAll(self):
        """
        Loads all items in my group from the database, returning a C{Deferred}
    ...
    def update(self, name, value):
        ...
        C{Deferred} that fires when the transaction is done.
        """
    ...
    def delete(self, *names):
        ...
        I{*names}, returning a C{Deferred} that fires when the transaction is
        done.

        If we are updating the search index, there's a nuance to the
        C{Deferred} processing. In that case, when the deletions are done, the
        C{Deferred} is fired and processing separately proceeds with dropping
        index entries for the deleted values. Here's how it works:
            ...
            function as the callback to its C{Deferred} B{d2}. Note that the
            defer-to-thread transaction keeps a reference to the C{Deferred}
            object it instantiates, so we don't have to do so for either B{d2}
            or B{d3}. Those references are merely defined in the method for
            code readability.
    ...
    def names(self):
        """
        Returns a C{Deferred} that fires with a list of the names of all items
