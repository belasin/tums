...
class Search(AccessBroker):
    ...
    def search(self, query, scope=None):
        ...
        first
            An integer specifying the position of the C{first} word in the
            matching text block within the specified section of the specified
            document.

        last
            An integer specifying the position of the C{last} word in the
            matching text block within the specified section of the specified
            document.

        words
            A list containing integer positions of words that triggered the
            match.

        document
            An integer specifying the document in which the text block was found.
            
        section
            An integer specifying the section of the document in which the text
            block was found.

        TODO: Initially the query just contains terms that must all be present
        in the item's text value, I{i.e.}, logical AND. Expand this method to
        parse the query for proximity operators etc.
        """
        results = []
