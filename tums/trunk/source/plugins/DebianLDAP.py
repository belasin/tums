...
class Plugin(object):
    ...
    def commonAuth(self):
        content = """auth    sufficient       pam_unix.so nullok_secure
...
bindpw %(bindpass)s\n""" % {
    ...
    'bindpass': config.LDAPPassword
        ...
        # Fix file linking
        os.system('rm /etc/libnss-ldap.conf /etc/libnss-ldap.secret /etc/pam_ldap.conf /etc/pam_ldap.secret > /dev/null 2>&1')
        ...
        os.system('chmod og-w /etc/ldap.conf') 
        os.system('chmod og+r /etc/ldap.conf') 
        os.system('chmod a+r /etc/ldap/schema/*') 
