...
class Plugin(object):
    ...
    def createAdmin(self):
        try:
            l = open('/usr/local/tcs/tums/adminpw', 'r')
            password = l.readlines()[0]
            l.close()
        except:
            password = 'admin123'
        submitData = {
            ...
            'userSettings.userPassword': password, 'mailSettings.vacation': None,
...
"""
        ...
        if config.ProxyConfig.get('contentfilter', False):
            os.system('/usr/local/tcs/tums/configurator --cfilter > /dev/null')
            os.system('/etc/init.d/dansguardian start > /dev/null')
