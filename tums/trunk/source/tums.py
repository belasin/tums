...
from Core import Utils, Auth, WebUtils, confparse, Shorewall, PBXUtils
from Core.PBXUtils import AGI
...
def pingDB(db, squidDb, cdrDB):
    ...
    db.pingDB().addCallbacks(noCB, noEB)
    squidDb.pingDB().addCallbacks(noCB, noEB)
    if cdrDB: 
        cdrDB.pingDB().addCallbacks(noCB, noEB)

    reactor.callLater(500, pingDB, db, squidDb, cdrDB)
...
def telDBPerformRout(telDB):
    ...
    for user, devs in fulDevList.items():
        telDB.setUserQueue(user,devs).addCallbacks(handSuc, handFal)
    reactor.callLater(60, telDBPerformRout, telDB)
...
def initialiseDB(db, squidDb, updateDb, telDB):
    ...
    def strap(_):
        ...
        reactor.callLater(2, pingDB, db, squidDb, telDB)
        #reactor.callLater(2, telDBPerformRout, telDB)
...
if __name__ == '__main__':
    f = fastagi.FastAGIFactory(AGI.authFactory)
